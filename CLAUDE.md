...
See the libez80.py header for the authoritative caveat list, audited 2026-06 against the CEmu CPU core (with file:line evidence). Highlights: suffixed (.SIS/.LIS) memory/stack ops execute in Z80 mode and hit `{MBASE, addr16}` instead of the 24-bit label address -- ALL such forms are broken on TI-OS and are no longer emittable; `.LIS SBC` flags are mode-width-correct (the historical failures were overflow-blind S-only signed compares); 8-bit register loads do not clear the pair's upper byte, so pointers assembled from 8-bit pieces need a 24-bit `LD rr,0` first, and loop counters next to pointers in RAM use full 24-bit loads/stores.
